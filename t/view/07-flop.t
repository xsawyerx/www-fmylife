...
SKIP: {
    ...
    cmp_ok( scalar @flop, '==', 15, 'Got flop 15 items' );
    foreach my $flop (@flop) {
    ...
    while ( my ( $format, $type_check ) = each %format_types ) {
        ...
        foreach my $check_type ( keys %check_types ) {
            ...
            @flop = $fml->flop( $check_types{$check_type} );
            cmp_ok( scalar @flop, '==', 15, 'Got flop 15 items' );
    ...
    SKIP: {
        ...
        $mock_obj->mock(
            ...
            sub {
                my $asked_url  = $_[1];
                my $needed_url = 'http://api.betacie.com/view/flop/3';
