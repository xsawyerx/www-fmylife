...
# checking usage of 'flop_month' method
# flop_month does not have the page parameter
...
SKIP: {
    ...
    my @flop_month = $fml->flop_month();

    foreach my $flop (@flop_month) {
        isa_ok( $flop, 'WWW::FMyLife::Item', 'Item is an object' );
    }

    # checking one of the items
    my $item       = shift @flop_month;
    my @attributes = qw(
    ...
    while ( my ( $format, $type_check ) = each %format_types ) {
        @flop_month = $fml->flop_month( { as => $format } );
